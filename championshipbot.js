    ...
    return {
        ...
        playButtonClickListener: function () {
            ...
            if (userInterface.server.value) {
                ...
                serverIP = userInterface.server.value;
                if (s.length === 2) {
                    window.force_ip = s[0];
...
(function (window, document) {
    ...
    window.nick.value = userInterface.loadPreference('savedNick', 'Slither.IO');
    nickname = window.nick.value;
    serverIP = "5.9.19.112:444";
...
function loadChat() {
    ...
    locationsRef.on('child_changed', function(data) {
        ...
        if (item.nick != nickname) {
            ...
            updateScore(item.nick, item.score);
        }
    });
...
function initializeUI() {
    ...
    divHeader.innerHTML = "<div class='menu-icon'><div class='dash-top'></div><div class='dash-bottom'></div><div class='circle'></div></div><span class='title'>Notifications</span>";

    divPanel.appendChild(divHeader);

    var divGroup = document.createElement("div");
    divGroup.className = "group";
    //    divGroup.innerHTML = "<input type=\"text\" id=\"chat_message\" /><input type='button' value='Send' onclick='sendChatMessage' />";

    var inputText = document.createElement("input");
    inputText.type = "text";
    inputText.value = "";
    inputText.id = "chat_message";
    inputText.onclick = sendChatMessage;
    ...
    divGroup.appendChild(sendButton);

    divPanel.appendChild(divGroup);

    var divChatHistory = document.createElement("div");
    divChatHistory.id = "chat_history";
...
function sendChatMessage() {
    var t = document.getElementById("chat_message");
    ...
    writeRecord(nickname, serverIP, t.value, 3);
    t.value = "";
}
...
function writeLocation(nick, x, y, score, isRemove) {
    ...
    var updates = {};
    if (isRemove) {
        friendsLeaderboard.style.display = "none";
        updates['/location/' + newPostKey] = null;
    } else {
        friendsLeaderboard.style.display = "inline";
...
function updateScore(nick, score) {
